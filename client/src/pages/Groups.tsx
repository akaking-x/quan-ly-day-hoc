            ...
            {formData.schedule.length > 0 && (
              <div className="space-y-2">
                {formData.schedule.map((s, i) => (
                  <div key={i} className="flex items-center gap-2 bg-blue-50 dark:bg-blue-900/30 px-3 py-2 rounded-lg text-sm">
                    <Badge variant="primary" size="sm">{dayNames[s.dayOfWeek]}</Badge>
                    <span className="font-medium dark:text-white">{s.startTime}-{s.endTime}</span>
                    <span className="text-gray-500 dark:text-gray-400 truncate flex-1">{s.subject}</span>
                    <button
                      type="button"
                      onClick={() => removeSchedule(i)}
                      className="p-1 text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/50 rounded transition-colors flex-shrink-0"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
